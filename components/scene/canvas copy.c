...
void canvas_save_buf_nvs()
{
    ...
    size_t buf_size = CANVAS_WIDTH * CANVAS_HEIGHT / 8;
    esp_err_t err = nvs_set_blob(nvs_handle, nvs_key, buf, buf_size);
    if (err != ESP_OK)
    {
    ...
    {
        ESP_LOGI(TAG, "Canvas buffer saved to NVS with key: %s", key);
    }
    display_mux_unlock();
}
void canvas_load_buf_nvs()
{
    ...
    else
    {
        ...
        ESP_LOGI(TAG, "Canvas buffer loaded from NVS with key: %s", key);
    }
    display_mux_unlock();
}